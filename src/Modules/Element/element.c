...
Element* Element_create(int type,char symbol,int color){
    ...
    element->color = color;
    element->properties = Properties_SetInitialPropertiesForType(type);
    return element;
}
...
int Element_setKey(Element* playerElement){
    return Properties_setKey(playerElement->properties);
}

int Element_getKey(Element* playerElement){
    return Properties_getKey(playerElement->properties);
}
